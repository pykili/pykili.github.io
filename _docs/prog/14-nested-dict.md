...
Реплики персонажей нужно брать из сценариев ЗВ, [ссылка на страницу датасета сценариев](https://www.kaggle.com/xvivancos/star-wars-movie-script).
...
7. В время с полуночи до часу ночи программа сообщает пользователю о том, что он выиграл или проиграл, напечатав имя персонажа ASCII-артом (генерировать его не надо, можно захардкодить).
