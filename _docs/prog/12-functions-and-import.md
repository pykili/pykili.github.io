...
Если название модуля слишком длинное и вы не хотите его писать каждый раз, когда юзаете какую-то функцию, вы можете импортировать этот модуль под псевдонимом с помощью as:
...
Иногда в модуле бывает много разных функций, а вам нужно использовать только что-то одно (и, например, использовать много раз), тогда проще импортировать какую-то предленную функцию из этого модуля и (1) чуть-чуть сэкономить память, (2) **короче синтаксис**. Тогда синтаксис импорта будет следующий:
...
Также можно импортировать из модуля всё, то есть все функции, переменные и классы. Тогда нам не нужно каждый раз писать название модуля и точку каждый раз. (Но эта практика не очень распространённая)
...
На самом деле любая программа может выступать в роди модуля для другой и импортироваться. Давайте напишем скрипт с парой функций и импортируем эти функции в другую программу.
