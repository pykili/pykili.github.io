...
Если в строке встречается специальный символ `\` (бэкслеш, обратная косая черта), то идущий за ним символ может интерполироваться особым образом.
...
Ещё часто используются `\t` (символ табуляции), а сам символ бэкслеш обозначается как `\\`.
