...
Все пользовались функцией `split()`:
...
В данном случае `maxsplit` - это именованный аргумент функции `split()`
...
**Нельзя** определить именованный аргумент без назначения ему начального значения. Можно только задать фиктивное значение:

```python
>>> def say_hello(name, status=''):
>>> 	print('Привет,', status, name)
>>>
>>> say_hello('Петр')
Привет,  Петр
